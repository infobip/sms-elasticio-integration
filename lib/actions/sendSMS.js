...
function sendSMS(msg, cfg) {
	...
	if (!senderAddress) {
        this.emit('error', new Error('Sender address is required'));
        this.emit('end');
        return;
	}
    if (!destinationAddress) {
        this.emit('error', new Error('Destination address is required'));
        this.emit('end');
        return;
    }
    if (!message) {
        this.emit('error', new Error('Message text is required'));
        this.emit('end');
        return;
    }
